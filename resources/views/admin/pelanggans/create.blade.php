...
<div class="max-w-2xl">
    <div class="bg-white rounded-lg shadow-md p-6">
        <form method="POST" action="{{ route('admin.pelanggans.store') }}">
            @csrf
            ...
            <div class="mb-4">
                ...
                <input type="text" name="telepon" id="telepon" class="w-full px-3 py-2 border rounded-lg focus:outline-none" style="border-color: {{ $errors->has('telepon') ? '#ef4444' : '#d1d5db' }}; outline: 2px solid {{ $errors->has('telepon') ? '#fca5a5' : '#56C5D0' }}; outline-offset: -1px;" value="{{ old('telepon') }}" required>
                @error('telepon') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="mb-4">
                <label for="email" class="block text-sm font-semibold text-gray-700 mb-2">Email</label>
                <input type="email" name="email" id="email" class="w-full px-3 py-2 border rounded-lg focus:outline-none" style="border-color: {{ $errors->has('email') ? '#ef4444' : '#d1d5db' }}; outline: 2px solid {{ $errors->has('email') ? '#fca5a5' : '#56C5D0' }}; outline-offset: -1px;" value="{{ old('email') }}">
                @error('email') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="mb-4">
                <label for="alamat" class="block text-sm font-semibold text-gray-700 mb-2">Alamat *</label>
                <textarea name="alamat" id="alamat" rows="3" class="w-full px-3 py-2 border rounded-lg focus:outline-none" style="border-color: {{ $errors->has('alamat') ? '#ef4444' : '#d1d5db' }}; outline: 2px solid {{ $errors->has('alamat') ? '#fca5a5' : '#56C5D0' }}; outline-offset: -1px;" required>{{ old('alamat') }}</textarea>
