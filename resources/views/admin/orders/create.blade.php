...
@section('header', 'Tambah Order Baru')
        ...
        <form method="POST" action="{{ route('admin.orders.store') }}">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="date" name="tanggal_order" id="tanggal_order" class="w-full px-3 py-2 border rounded-lg focus:outline-none" style="border-color: {{ $errors->has('tanggal_order') ? '#ef4444' : '#d1d5db' }}; outline: 2px solid {{ $errors->has('tanggal_order') ? '#fca5a5' : '#56C5D0' }}; outline-offset: -1px;" value="{{ old('tanggal_order') }}">
                ...
                <div>
                    ...
                    <input type="number" step="0.1" name="berat" id="berat" class="w-full px-3 py-2 border rounded-lg focus:outline-none" style="border-color: {{ $errors->has('berat') ? '#ef4444' : '#d1d5db' }}; outline: 2px solid {{ $errors->has('berat') ? '#fca5a5' : '#56C5D0' }}; outline-offset: -1px;" value="{{ old('berat') }}">
            ...
            <div class="mb-4">
                ...
                <textarea name="catatan" id="catatan" rows="3" class="w-full px-3 py-2 border rounded-lg focus:outline-none" style="border-color: {{ $errors->has('catatan') ? '#ef4444' : '#d1d5db' }}; outline: 2px solid {{ $errors->has('catatan') ? '#fca5a5' : '#56C5D0' }}; outline-offset: -1px;">{{ old('catatan') }}</textarea>
